//
//  VideoClientNetworkReachability.swift
//  VideoClient
//
//  Created by Krishna Picart on 7/5/17.
...
class VideoClientNetworkReachability: NSObject {
    ...
    static func networkReachabilityForInternetConnection() -> VideoClientNetworkReachability? {
        var zeroAddress = sockaddr_in()
        zeroAddress.sin_len = UInt8(MemoryLayout.size( ofValue:zeroAddress))
        
        zeroAddress.sin_family = sa_family_t(AF_INET)
        return VideoClientNetworkReachability(hostAddress: zeroAddress)
    }
    
    static func networkReachabilityForLocalWiFi() -> VideoClientNetworkReachability? {
        ...
        return VideoClientNetworkReachability(hostAddress: localWifiAddress)
    }
    ...
    func startNotifier() -> Bool {
        ...
        guard let reachability = networkReachability, SCNetworkReachabilitySetCallback(reachability, { (target: SCNetworkReachability, flags: SCNetworkReachabilityFlags, info: UnsafeMutableRawPointer?) in
            ...
            if let currentInfo = info {
                ...
                if infoObject is VideoClientNetworkReachability {
                    let networkReachability = infoObject as! VideoClientNetworkReachability
